...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const payload = { id: user.id, roles, scopes };
    ...
    res.json({
      accessToken,
      refreshToken,
      user: { id: user.id, fullName: user.fullName, roles, username: user.username },
    });
  })
);
...
authRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!user || user.status !== "ACTIVE") throw new AppError("المستخدم غير موجود أو غير فعّال", 401);
    ...
    const accessToken = jwt.sign({ id: user.id, roles, scopes }, env.jwtAccessSecret, {
